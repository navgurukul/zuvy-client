...
const BLOOMS: { id: string; label: string; hint: string }[] = [
    { id: 'remember', label: 'Remember', hint: 'Recall facts & concepts' },
    {
        id: 'understand',
        label: 'Understand',
        hint: 'Explain ideas or concepts',
    },
...
const BLOOM_CLASSES: Record<string, string> = {
    remember:
        'border-slate-200 bg-slate-50 text-slate-600 data-[state=on]:border-slate-500 data-[state=on]:bg-slate-100 data-[state=on]:text-slate-900',
    understand:
        'border-blue-200 bg-blue-50 text-blue-600 data-[state=on]:border-blue-500 data-[state=on]:bg-blue-100 data-[state=on]:text-blue-900',
    ...
    analyze:
        'border-amber-200 bg-amber-50 text-amber-700 data-[state=on]:border-amber-500 data-[state=on]:bg-amber-100 data-[state=on]:text-amber-900',
    evaluate:
        'border-red-200 bg-red-50 text-red-600 data-[state=on]:border-red-500 data-[state=on]:bg-red-100 data-[state=on]:text-red-900',
...
interface AiFormState {
    ...
    topicMode: 'existing' | 'custom'
    subtopics: string[]
    subtopicInput: string
    subtopicError: string
...
function AiFormFields({
    ...
    const totalCount = Object.values(form.difficultyCounts).reduce(
        (s, n) => s + n,
        0
    )
    const selectedTopic = evalTopics?.find(
        (t) => t.id.toString() === form.topic
    )
    const predefinedSubtopics: string[] =
        form.topicMode === 'existing'
            ? ((selectedTopic?.subtopic as string[] | undefined) ?? [])
            : []
    const customSubtopics = form.subtopics.filter(
        (s) => !predefinedSubtopics.includes(s)
    )
    const patchCount = (d: string, val: number) =>
        patch({
            difficultyCounts: {
                ...form.difficultyCounts,
                [d]: Math.max(0, Math.min(20, val)),
            },
        })
    ...
    const addCustomSubtopic = () => {
        ...
        if (!v) return
        const existing = [...predefinedSubtopics, ...form.subtopics].find(
            (s) => normalise(s) === normalise(v)
        )
        if (existing) {
        ...
        patch({
            subtopics: [...form.subtopics, v],
            subtopicInput: '',
            subtopicError: '',
        })
    }

    const removeSubtopic = (s: string) =>
        patch({ subtopics: form.subtopics.filter((x) => x !== s) })
    return (
        <div className="flex flex-col gap-5">
                    ...
                    <Card className="bg-muted/40">
                        <CardContent className="p-3.5">
                            ...
                            <Input
                                ...
                                onChange={(e) =>
                                    patch({ customTopic: e.target.value })
                                }
                            ...
                            <Textarea
                                ...
                                onChange={(e) =>
                                    patch({ customTopicDesc: e.target.value })
                                }
                            ...
                            <Button
                                ...
                                onClick={() =>
                                    submitTopic({
                                        name: form.customTopic,
                                        description: form.customTopicDesc,
                                        subtopic: {},
                                    })
                                }
            ...
            <div>
                ...
                <div className="flex items-center gap-2">
                    <Input
                        ...
                        placeholder={
                            predefinedSubtopics.length > 0
                                ? 'Add a subtopic not in the list above…'
                                : 'Type a subtopic and press Enter…'
                        }
                    />
                    <Button
                        className="mt-2"
            ...
            <div>
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <p className="pr-0.5 text-right text-xs text-muted-foreground">
                        Total:{' '}
                        <strong
                            className={totalCount > 0 ? 'text-foreground' : ''}
                        >
                            {totalCount}
                        </strong>{' '}
                        question{totalCount !== 1 ? 's' : ''} · max 20 per
                        difficulty
                    </p>
                </div>
            </div>
            ...
            <div>
                <FieldLabel>
                    ...
                    <span className="font-normal normal-case tracking-normal">
                        — recommended · paste lesson text to make questions
            ...
            <div>
                ...
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                    <strong>Remember</strong> for recalling facts and concepts,{' '}
                    <strong>Understand</strong> for explaining ideas or
                    concepts, <strong>Apply</strong> for using knowledge in a
                    new situation, <strong>Analyze</strong> for drawing
                    connections and identifying patterns,{' '}
                    <strong>Evaluate</strong> for justifying a decision or
                    course of action, and <strong>Create</strong> for producing
...
export default function AiQuestionFormPage({
    ...
    const activeTopic =
        form.topicMode === 'custom' ? form.customTopic.trim() : form.topic
    // const activeTopicId = form.topicMode === 'custom' ? '' : form.topic
    const activeTopicName =
        form.topicMode === 'custom' ? form.customTopic.trim() : form.topicName
    const totalCount = Object.values(form.difficultyCounts).reduce(
        (s, n) => s + n,
        0
    )
    ...
    const handleSubmit = async () => {
        const calcPct = (val: number) =>
            totalCount > 0 ? Math.round((val / totalCount) * 100) : 0
        const counts = {
        ...
        const payload = {
            ...
            topicDescription:
                form.topicMode === 'custom' ? form.customTopicDesc : '',
    ...
    return (
        <div className="mx-auto max-w-2xl px-6 py-8">
            <h1 className="mb-1 flex text-2xl font-bold">
            ...
            <span className="mb-6 block text-left text-sm text-muted-foreground">
                ...
                queue — an SME must approve them before they appear in
                assessment pools.
            </span>
