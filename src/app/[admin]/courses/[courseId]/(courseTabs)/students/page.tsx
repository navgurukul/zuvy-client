...
const StudentsPage = ({ params }: { params: any }) => {
  ...
  const { students, totalPages, currentPage, limit, offset, search, totalStudents, setStudents, fetchStudentData } = useStudentData(params.courseId)
      ...
      <div>
        <div className="flex flex-col md:flex-row justify-between items-center gap-y-4">
          <div className="relative w-full md:w-1/2 lg:w-1/4">
            <SearchBox placeholder="Search students..." fetchSuggestionsApi={fetchSuggestionsApi} fetchSearchResultsApi={fetchSearchResultsApi} defaultFetchApi={defaultFetchApi} getSuggestionLabel={(student) => (<div><div className="font-medium">{student.name}</div><div className="text-sm text-gray-500">{student.email}</div></div>)} inputWidth="w-full" />
          </div>
          ...
          <div className="flex w-full flex-row mt-2 md:flex-row items-center gap-x-4 gap-y-2 md:w-auto">
            ...
            <Dialog open={isSingleStudentOpen} onOpenChange={(open) => { setIsSingleStudentOpen(open); if (!open) setStudentData({ name: '', email: '' }); }}>
              <DialogTrigger asChild>
                <Button className="flex-1 text-gray-800 border border-input bg-background hover:bg-accent hover:text-accent-foreground"><UserPlus className="h-4 w-4 mr-2" />Add Single Student</Button>
              </DialogTrigger>
              <DialogOverlay />
              <AddStudentsModal message={false} id={params.courseId || 0} batch={false} batchId={0} setStudentData={setStudentData} studentData={studentData} modalType="single" />
            </Dialog>

            {/* Bulk Upload Button */}
            <Dialog open={isOpen} onOpenChange={(open) => { setIsOpen(open); if (!open) setStudentData({ name: '', email: '' }); }}>
              <DialogTrigger asChild>
                <Button className="flex-1 bg-primary hover:bg-primary-dark shadow-4dp"><FileSpreadsheet className="h-4 w-4 mr-2" />Bulk Upload</Button>
              </DialogTrigger>
              <DialogOverlay />
              <AddStudentsModal message={false} id={params.courseId || 0} batch={false} batchId={0} setStudentData={setStudentData} studentData={studentData} modalType="bulk" />
            </Dialog>
          </div>
        </div>

        <div className='flex flex-row justify-end mt-6'>
          {selectedRows.length > 0 && (<><AlertDialogDemo setSelectedRows={setSelectedRows} userId={userIds} bootcampId={batchData && params.courseId} title="Are you absolutely sure?" description={`This action cannot be undone. This will permanently remove the ${selectedRows.length > 1 ? 'students' : 'student'} from the bootcamp`} /><ComboboxStudent batchData={newBatchData || []} bootcampId={batchData && params.courseId} selectedRows={selectedRows} fetchStudentData={fetchStudentDataForBatch} /></>)}
        </div>
        ...
        <div className="flex flex-col md:flex-row items-center gap-y-4 md:gap-x-4 md:gap-y-0 mt-10">
          ...
          <div className="w-full sm:w-[160px] mt-2">
            <Select value={enrolledDateFilter} onValueChange={handleEnrolledDateFilterChange}>
              <SelectTrigger className="text-sm w-full"><SelectValue placeholder="Enrolled Dates" /></SelectTrigger>
              <SelectContent><SelectItem value="all">All Enrolled</SelectItem><SelectItem value="today">Today</SelectItem><SelectItem value="yesterday">Yesterday</SelectItem><SelectItem value="thisWeek">This Week</SelectItem><SelectItem value="thisMonth">This Month</SelectItem><SelectItem value="last3months">Last 3 Months</SelectItem><SelectItem value="last6months">Last 6 Months</SelectItem><SelectItem value="lastyear">Last Year</SelectItem><SelectItem value="older">Older</SelectItem></SelectContent>
            </Select>
          </div>
 ...
 <SelectItem value="older">Older</SelectItem>
          ...
          <div className="w-full sm:w-[160px]">
            <Input type="text" placeholder="Attendance number" value={attendanceFilter} onChange={handleAttendanceFilterChange} className="text-sm w-full" />
          </div>
        </div>

        <div>
          <div className="mt-5">
            <DataTable data={students} columns={columns} setSelectedRows={setSelectedRows} />
          </div>

          {/* Use the imported DataTablePagination component */}
          <DataTablePagination totalStudents={totalStudents} lastPage={totalPages} pages={totalPages} fetchStudentData={fetchFilteredData} />
        </div>
      </div>
    </div>
  )
