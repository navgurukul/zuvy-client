...
import {
    ...
    BatchOption,
    SelecteItem,
} from '@/app/[admin]/courses/[courseId]/batch/[batchId]/CourseBatchesType'
...
const BatchesInfo = ({
    ...
    const fetchBatches = useCallback(
        async (courseId: string) => {
            try {
                const response = await api.get<{ data: Batch[] }>(
                    `/bootcamp/batches/${courseId}`
                )
                const batchData: BatchOption[] = response.data.data?.map(
                    (data: any) => {
                        return {
                            value: data.id,
                            label: data.name,
    ...
    const fetchStudentSuggestions = useCallback(
        async (query: string) => {
            try {
                ...
                return (
                    response.data.modifiedStudentInfo?.map(
