        ...
        <Card className="bg-gradient-to-r from-card to-muted/30 border border-border rounded-2xl shadow-lg">
          <CardHeader className="flex flex-row items-center gap-x-4 p-6">
            <Avatar className="h-14 w-14 shadow-md ring-2 ring-muted-foreground/20">
              ...
              <AvatarFallback className="font-bold bg-muted">CN</AvatarFallback>
            </Avatar>
            <div>
              <CardTitle className="text-2xl font-bold">{user?.name} - Quiz Questions Report</CardTitle>
              <p className="text-left text-muted-foreground text-md">Detailed breakdown of your quiz performance</p>
            </div>
          </CardHeader>
        </Card>

        <h1 className="text-left font-semibold text-2xl mt-4">Overview</h1>

        <div className="space-y-10">
          {quizQuestionDetails?.mcqs?.map((quizDetail: QuizDetails, index: number) => {
            const { submissionsData } = quizDetail;
            const chosenOption = submissionsData?.chosenOption;
            const isAttempted = chosenOption !== null && chosenOption !== undefined;
            ...
            return (
              ...
              >
                ...
                <CardContent className="p-6 space-y-4">
                  {Object.entries(quizDetail.options).map(([key, option]) => {
                    const optionKey = parseInt(key);
                    const isCorrect = optionKey === quizDetail.correctOption;
                    const isChosen = optionKey === chosenOption;

                    const showChecked = isCorrect || isChosen;
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <span className="break-words">{option as string}</span>
                      </div>
                    );
                  })}
