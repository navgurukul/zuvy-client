...
const parseIsoToDateAndTime = (isoString?: string | null) => { // Allow null for isoString
    if (isoString) { // This check handles undefined and null correctly (both are falsy)
        try {
...
const PublishAssessmentDialog: React.FC<PublishAssessmentDialogs> = ({
    ...
    const [publishDate, setPublishDate] = useState<Date>(parseIsoToDateAndTime(initialPublishDate).date);
    const [startDate, setStartDate] = useState<Date>(parseIsoToDateAndTime(initialStartDate).date);
    const [endDate, setEndDate] = useState<Date>(parseIsoToDateAndTime(initialEndDate).date);
    const [endNowDate, setEndNowDate] = useState<Date>(parseIsoToDateAndTime(initialEndDate).date); // Also use initialEndDate for this

    const [publishTime, setPublishTime] = useState(parseIsoToDateAndTime(initialPublishDate).time);
    const [startTime, setStartTime] = useState(parseIsoToDateAndTime(initialStartDate).time);
    const [endTime, setEndTime] = useState(parseIsoToDateAndTime(initialEndDate).time);
    const [endNowTime, setEndNowTime] = useState(parseIsoToDateAndTime(initialEndDate).time); // And for this
    ...
    const [scheduleCalendarOpen, setScheduleCalendarOpen] = useState<boolean[]>([false, false, false]);
    ...
    const combineDateTime = (date: Date, time: string): Date => {
        ...
        const newDateTime = new Date(date.getFullYear(), date.getMonth(), date.getDate(), hours, minutes, 0, 0);
        return newDateTime;
    };

    const validateTime = (publishTimeStr: string, startTimeStr: string, endTimeStr: string) => {
        const now = new Date();
        ...
        const publishDateTime = publishDate && publishTimeStr ? combineDateTime(publishDate, publishTimeStr) : null;
        const startDateTime = startDate && startTimeStr ? combineDateTime(startDate, startTimeStr) : null;
        const endDateTime = endDate && endTimeStr ? combineDateTime(endDate, endTimeStr) : null;
        ...
        if (publishDateTime) {
            ...
            if (startDateTime && publishDateTime > startDateTime) {
                currentPublishTimeError = 'Publish time cannot be after start time';
            }
        }
        ...
        if (startDateTime) {
            if (startDateTime < now) {
                currentStartTimeError = 'Start date and time cannot be in the past';
            }
        }
        
        // Rule: If publish date/time is provided, start date/time is mandatory
        if (publishDateTime && !startDateTime) {
            const msg = 'Start date and time are required if publish date and time are set.';
        ...
        return !(currentPublishTimeError || currentStartTimeError || currentEndTimeError || currentScheduleActionError);
    };
    
    const validateNowEndTime = (date: Date, time: string) => {
        const now = new Date();
        const endDateTime = time ? combineDateTime(date, time) : null;
        if (!time) { // If time is cleared, remove the error
            ...
            return false;
        } else { // If time is valid, remove the error
            setErrors((prev) => ({ ...prev, endNowTime: undefined }));
            return true;
        }
    };
    ...
    const handleScheduleAssessment = () => {
        ...
        if (publishDate && publishTime && (!startDate || !startTime)) {
            setErrors(prev => ({...prev, scheduleAction: 'Start date and time are required if publish date and time are set.', startTime: 'Required when publish date/time is set.'}));
            return;
        }
        ...
        const scheduleData: PublishData = {
            action: 'schedule',
            publishDateTime: (publishDate && publishTime) ? combineDateTime(publishDate, publishTime).toISOString() : undefined,
            startDateTime: (startDate && startTime) ? combineDateTime(startDate, startTime).toISOString() : undefined,
            endDateTime: (endDate && endTime) ? combineDateTime(endDate, endTime).toISOString() : undefined,
        };
        onSave(scheduleData);
    ...
    const handlePublishNow = () => {
        ...
        if (endNowTime) { // End time is optional for publish now
            if (!validateNowEndTime(endNowDate, endNowTime)) {
                isValid = false;
                // validateNowEndTime sets the error
            } else {
                 endDateTimeIso = combineDateTime(endNowDate, endNowTime).toISOString();
            }
        }
    ...
    }, [publishDate, publishTime, startDate, startTime, endDate, endTime, initialPublishDate, initialStartDate, initialEndDate]);
        ...
        <>
            ...
            <Tabs value={tab} onValueChange={(val) => setTab(val as 'schedule' | 'now' | 'draft')} // Added 'draft' to cast
                    ...
                    {[
                        ...
                        <div key={i} className="space-y-1">
                            ...
                            <div className="flex items-center gap-4"> {/* Changed items-start to items-center */}
                                ...
                                <div className="flex-1"> {/* Removed flex flex-col */}
                                    <Input
                                        type="time"
                                    ...
                                    />
                                    {error && <p className="text-xs text-red-500 leading-snug absolute">{error}</p>}
                                </div>
                            </div>
                        </div>
                                    ...
                                    <DialogContent className="w-auto p-4">
                                        <Calendar
                                            ...
                                            onSelect={(selectedDate) => {
                                                setEndNowDate(selectedDate || new Date());
                            ...
                            <div className="flex-1 relative">
                                <Input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (e.target.value) validateNowEndTime(endNowDate, e.target.value);
                                        else setErrors(prev => ({...prev, endNowTime: undefined}));
                                    }}
                                />
                                {errors.endNowTime && <p className="text-xs text-red-500 leading-snug absolute">{errors.endNowTime}</p>}
                            </div>
                        </div>
                    </div>
                    <div className="flex justify-end gap-2 pt-4">
                ...
                <TabsContent value="draft" className="mt-4 space-y-4">
                    <p className="text-sm text-muted-foreground">
                        Moving this assessment to draft will unpublish it and remove any scheduled dates. 
