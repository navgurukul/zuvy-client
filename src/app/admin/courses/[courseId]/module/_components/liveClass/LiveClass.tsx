...
// import DeleteConfirmationDialog from '@/app/admin/courses/[courseId]/module/_components/liveClass/deleteLiveClass'
...
//     const chapterContentsCount = chapterData?.contents?.length || 0;
...
//     try {
//       setLoading(true)
//       await api.delete(`/classes/delete/${session.meetingId}`)

//       if (chapterContentsCount === 1 && chapterId) {
//         await api.delete(`/chapters/delete/${chapterId}`)
//         toast.success('Chapter and session deleted successfully')
//       } else {
...
//     } catch (error) {
//       toast.error('Unable to delete session or chapter')
//     } finally {
//       setLoading(false)
      ...
      <div>
        <CardHeader className="pb-4">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              {content?.description && (
                <p className="text-sm text-left text-muted-foreground mb-3">{content.description}</p>
              )}
              <div className="flex items-center gap-2 mb-3">
                <Badge
                  variant="secondary"
        ...
        <CardContent className="pt-0 space-y-4">
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3 text-sm text-muted-foreground">
              <Users className="w-4 h-4 text-primary" />
              <span className="font-medium">Created by {session.creator}</span>
            </div>
          </div>
          ...
          <div className="pt-4 ">
            ...
            session.s3link !== 'not found' ? (
              ...
              <Button
                className={`w-full font-medium py-2.5 transition-all duration-200 bg-success-dark text-white ${
                  session.status.toLowerCase() !== 'ongoing' ? 'opacity-50 cursor-not-allowed' : ''
                }`}
                onClick={() => {
                  if (session.status.toLowerCase() !== 'ongoing') return;
                  window.open(session.hangoutLink, '_blank');
...
onConfirm={async () => {
  try {
    setLoading(true);
    await api.delete(`/classes/delete/${session.meetingId}`);
    const updatedContentsCount = chapterData?.contents?.filter((c: { id: any; }) => c.id !== content.id)?.length || 0;
    if (updatedContentsCount === 0 && chapterId) {
    await api.delete(`/content/deleteChapter/${moduleId}?chapterId=${chapterId}`);
    await api.delete(`/content/deleteChapter/${moduleId}`, {
  data: { chapterId },
});
