...
const EditSessionDialog: React.FC<EditSessionProps> = (props) => {
  ...
  const { sessionTitle, description, startDate, startTime, endTime } = form.watch();
  console.log('endTime :', endTime);
  console.log('startTime:', startTime);
  const isSubmitDisabled = !(sessionTitle && description && startDate && startTime && endTime);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      const transformedData = {
        ...
        try {
            const response = await api.patch(`/classes/update/${props.meetingId}`, transformedData);
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                                    ...
                                    <Popover>
                                        <PopoverTrigger asChild>
                                            <FormControl>
                                                <Button
                                                    variant={'outline'}
                                                    className={`w-[230px] text-left font-normal ${!field.value && 'text-muted-foreground'}`}
                                                >
                        ...
                        <div className="flex items-center gap-x-4">
                            <FormField control={form.control} name="startTime" render={({ field }) => (
                                <FormItem className="text-left flex flex-col">
                                    ...
                                    <FormControl>
                                        <Input placeholder="startTime" {...field} type="time" />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )} />
                          ...
                          <FormField control={form.control} name="endTime" render={({ field }) => (
                              <FormItem className="text-left flex flex-col">
                                  ...
                                  <FormControl>
                                      <Input placeholder="endTime" {...field} type="time" />
                                  </FormControl>
                                  <FormMessage />
                              </FormItem>
                          )} />
                        ...
                        <div className="flex justify-end">
                            <Button disabled={isSubmitDisabled} variant={'secondary'} type="submit">
                                Save Changes
                            </Button>
                            <DialogClose asChild>
