...
const SubmissionComponent = (props: Props) => {
    const handleDownloadPdf = async (id: any) => {
        ...
        async function fetchData() {
            try {
                ...
                console.log("catch",response.data.data[0])
                ...
                autoTable(doc, {
                    ...
                    body: rows.map((row: { name: string; email: string; status: string }) => [row.name, row.email, row.status]), // Ensure status is used here
                    startY: 25,
    ...
    const submissionPercentage =
        props.totalSubmissions > 0 ? props.studentsSubmitted / props.totalSubmissions : 0
    const isDisabled = props.studentsSubmitted === 0;

    // Check if there are no submissions
    const hasSubmissions = props.totalSubmissions > 0 && props.studentsSubmitted > 0
                ...
                <div className="flex justify-between items-center">
                    <div className="text-start flex gap-x-2">
                        <div className="flex items-center justify-center">
                            <div
                                className={`w-2 h-2 rounded-full flex items-center justify-center ${submissionPercentage >= 0.8
                                        ? 'bg-green-300'
                                        : submissionPercentage <= 0.8 && submissionPercentage >= 0.5
                                            ? 'bg-yellow-300'
                                            : 'bg-red-500'
                                    }`}
                            ></div>
                        ...
                        {hasSubmissions ? (
                            ...
                            >
                                <h3 className="font-semibold cursor-pointer">Submissions</h3>
                            </Link>
                        ) : (
