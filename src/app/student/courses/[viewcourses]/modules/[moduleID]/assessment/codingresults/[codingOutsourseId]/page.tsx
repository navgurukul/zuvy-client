    ...
    async function getCodingSubmissionsData(codingOutsourseId: any) {
        try {
            const res = await api.get(
                `codingPlatform/PracticeCode?studentId=${userID}&codingOutsourseId=${codingOutsourseId}`
            )
                ...
                <div className="grid grid-cols-2 gap-5 w-full max-w-7xl">
                    <div className="bg-white shadow-md rounded-lg p-6">
                        ...
                        <div className="mb-4">
                            ...
                            <p>
                                <strong>Tags:</strong> {questionInfo?.tags}
                            </p>
                        </div>
                        <div className="mb-4">
                            <h3 className="text-xl font-semibold">Examples:</h3>
                            {questionInfo?.examples.map(
                    ...
                    <div className="bg-white shadow-md rounded-lg p-6">
                        ...
                        <div className="mb-4">
                            ...
                            <pre className="bg-gray-100 p-4 rounded">
                                {decodeBase64(shapecode?.source_code)}
                            </pre>
                        </div>
                        <div className="mb-4">
                            <h4 className="text-lg font-semibold">
                            ...
                            <p>
                                <strong>Input:</strong>{' '}
                                {decodeBase64(shapecode?.stdin)}
                            </p>
                            <p>
                                <strong>Expected Output:</strong>{' '}
                                {decodeBase64(shapecode?.expected_output)}
                            </p>
                            <p>
                                <strong>Actual Output:</strong>{' '}
                                {decodeBase64(shapecode?.stdout)}
                            </p>
                            {shapecode?.stderr && (
                                <div className="text-red-500">
                                    <strong>Error Output:</strong>{' '}
                                    {decodeBase64(shapecode?.stderr)}
                                </div>
                            )}
                        </div>
                        <p>
                            <strong>Memory Used:</strong> {shapecode?.memory} KB
                        </p>
                        <p>
                            <strong>Execution Time:</strong> {shapecode?.time}{' '}
                            seconds
                        </p>
                    </div>
                </div>
