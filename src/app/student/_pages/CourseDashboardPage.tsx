...
const CourseDashboard = ({ courseId }: { courseId: string }) => {
    ...
    const {
        ...
        error: progressError,
    } = useBootcampProgress(courseId)
    const {
        modules: apiModules,
        loading: modulesLoading,
        error: modulesError,
    } = useAllModulesForStudents(courseId)
    const {
        upcomingEventsData,
        loading: eventsLoading,
        error: eventsError,
    } = useUpcomingEvents(courseId)
    const {
        completedClassesData,
        loading: classesLoading,
        error: classesError,
    } = useCompletedClasses(courseId)
    const {
        latestCourseData,
        loading: latestCourseLoading,
        error: latestCourseError,
    } = useLatestUpdatedCourse(courseId)
    ...
    const instructorAvatar =
        progressData?.instructorDetails?.instructorProfilePicture || ''
    ...
    const validInstructorAvatar = isValidImageUrl(instructorAvatar)
        ? instructorAvatar
        : ''
    const courseName = progressData?.data?.bootcampTracking?.name || ''
    const courseDescription =
        progressData?.data?.bootcampTracking?.description || ''
    ...
    const modulesToShow = showAllModules
        ? modulesToDisplay
        : modulesToDisplay.slice(0, 7)
    ...
    const formatDateRange = () => {
        ...
        return `From ${today.toLocaleDateString(
            'en-US',
            formatOptions
        )} to ${seventhDay.toLocaleDateString('en-US', formatOptions)}`
    }

    const getEventType = (
        type: string
    ): 'Live Class' | 'Assessment' | 'Assignment' | 'unknown' => {
    ...
    const getModuleDescription = (module: Module) => {
        return (
            module.description ||
            'Learn essential concepts and build practical skills.'
        )
    }
    ...
    const getContentCount = (module: ModuleContentCounts) => {
        ...
        if (module.codingProblemsCount > 0)
            counts.push(
                `${module.codingProblemsCount} Coding Problem${
                    module.codingProblemsCount > 1 ? 's' : ''
                }`
            )
        if (module.articlesCount > 0)
            counts.push(
                `${module.articlesCount} Article${
                    module.articlesCount > 1 ? 's' : ''
                }`
            )
        if (module.formCount > 0)
            counts.push(
                `${module.formCount} Form${module.formCount > 1 ? 's' : ''}`
            )
                ...
                <div className="w-full rounded-b-lg shadow-8dp bg-gradient-to-br from-primary/8 via-background to-accent/8 border-b border-border/50">
                    <div className="max-w-7xl mx-auto p-6 md:p-8">
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex-1">
                                        ...
                                        <TruncatedDescription
                                            ...
                                            maxLength={250}
                                            className="text-base md:text-lg text-muted-foreground mb-4 text-left"
                                        />
                        ...
                        <div className="md:hidden mb-6">
                            ...
                            />
                            <h1 className="text-2xl font-heading font-bold mb-2 text-left">
                                {courseName}
                            </h1>
                            <TruncatedDescription
                                text={courseDescription}
                                maxLength={150}
                                className="text-base text-muted-foreground mb-4 text-left"
                            />
                            <div className="flex items-center gap-2 mb-4">
                                {/* <Avatar className="w-8 h-8">
                  <AvatarImage src={validInstructorAvatar || '/logo.PNG'} />
                                    ...
                                    {modulesToShow.length > 0 ? (
                                        modulesToShow.map((module: any) => {
                                            ...
                                            const isCurrentModule =
                                                latestCourseData?.moduleId ===
                                                module.id
                                            const isCompleted =
                                                moduleProgress === 100
                                            const isLocked = module.isLock
                                            const upcomingChapterId =
                                                latestCourseData?.newChapter
                                                    ?.id || 1
                                            ...
                                            return (
                                                <Card
                                                    ...
                                                    className={`shadow-4dp   ${
                                                        ...
                                                        !isLocked
                                                            ? 'cursor-pointer hover:shadow-8dp transition-shadow'
                                                            : 'cursor-not-allowed'
                                                    }`}
                                                >
                                                    <CardContent
                                                    ...
                                                    >
                                                        <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4">
                                                            <div className="flex-1 text-left">
                                                                {isCurrentModule && (
                                                                    <Badge className="mb-2 bg-primary-light text-primary border-primary/20 self-start hover:bg-primary-light/80">
                                                                        Current
                                                                        Module
                                                                    </Badge>
                                                                )}
                                                                {isCompleted && (
                                                                    <Badge className="mb-2 bg-success-light text-success border-success/20 self-start hover:bg-success-light/80">
                                                                        Completed
                                                                    </Badge>
                                                                )}
                                                                {/* {isLocked && (
                                  <Badge className="mb-2 bg-muted text-muted-foreground border-muted/20 self-start flex items-center gap-1">
                                    ...
                                    Locked
                                                                ...
                                                                <TruncatedDescription
                                                                    ...
                                                                    className="text-muted-foreground mb-3 text-sm text-left "
                                                                />
                                                                ...
                                                                <TooltipProvider>
                                                                    <Tooltip>
                                                                        ...
                                                                        >
                                                                            <div>
                                                                                <Button
                                                                                    className={`px-6 font-semibold bg-  ${
                                                                                        isLocked ||
                                                                                        (module.typeId !==
                                                                                            2 &&
                                                                                            !module.ChapterId)
                                                                                            ...
                                                                                            : ` text-primary hover:underline hover:underline-offset-4 ${
                                                                                                  isCurrentModule
                                                                                                      ? 'border-2 border-primary bg-primary text-white hover:no-underline'
                                                                                                      : ''
                                                                                              } `
                                                                                    }`}
                                                                                    disabled={
                                                                                ...
                                                                                >
                                                                                    ...
                                                                                    2 ? (
                                                                                        <Link
                                                                                            ...
                                                                                            href={`${
                                                                                                isCurrentModule
                                                                                                    ? `/student/course/${courseId}/modules/${module.id}?chapterId=${upcomingChapterId}`
                                                                                                    : `/student/course/${courseId}/modules/${module.id}?chapterId=${module.ChapterId}`
                                                                                            }`}
                                                                                        >
                                                                                            {getModuleCTA(
                                                                                                module,
                                                                                        ...
                                                                                        <Link
                                                                                            href={`/student/course/${courseId}/projects?moduleId=${module.id}&projectId=${module.projectId}`}
                                                                                            className={`${
                                                                                                isCurrentModule
                                                                                                    ? 'text-white'
                                                                                                    : ''
                                                                                            } text-sm text-muted-foreground text-primary hover:underline`}
                                                                                        >
                                                                                            View
                                                                                            Project
                                                                                        </Link>
                          ...
                          tion Button - Mobile: bottom */}
                                                            ...
                                                            <TooltipProvider>
                                                                <Tooltip>
                                                                    ...
                                                                    >
                                                                        <div>
                                                                            <Button
                                                                                className={`px-6 mb-2 font-semibold bg- ${
                                                                                    isLocked ||
                                                                                    (module.typeId !==
                                                                                        2 &&
                                                                                        !module.ChapterId)
                                                                                        ...
                                                                                        : ` text-primary hover:underline  ${
                                                                                              isCurrentModule
                                                                                                  ? 'border-2 border-primary bg-primary text-white hover:no-underline w-full'
                                                                                                  : ''
                                                                                          } `
                                                                                }`}
                                                                                disabled={
                                        ...
                                        <div className="mb-6 text-left flex flex-col justify-end w-full items-center">
                                            ...
                                            <p className="text-sm text-left w-full text-muted-foreground leading-relaxed">
                                                No modules created for this
                                                course yet, Please check back
                                                later.
                                            </p>
                                        </div>
                                    )}

                                    {modulesToDisplay.length > 7 &&
                                        !showAllModules && (
                                    ...
                                    {eventsLoading ? (
                                      ...
                                      upcomingEventsData.events.length > 0 ? (
                                        <div className="space-y-4">
                                                                ...
                                                                href={`${
                                                                    ...
                                                                    'ongoing'
                                                                        ? (
                                                                              ...
                                                                              .hangoutLink
                                                                        : `/student/course/${courseId}/modules/${item.moduleId}?chapterId=${item.chapterId}`
                                                                }`}
                                                            >
                                                                <div>
                                                                    <div className="flex items-start gap-4">
                                                                            ...
                                                                            <Badge
                                                                                className={`my-2 hover:text-white ${
                                                                                    ...
                                                                                    'Live Class'
                                                                                        ...
                                                                                        : item.type ===
                                                                                          'Assessment'
                                                                                        ? 'bg-warning-light text-warning border-warning/20 hover:bg-warning'
                                                                                        : 'bg-info-light text-info border-info/20 hover:bg-info'
                                                                                } `}
                                                                            >
                                                                                {
                                                                                    item.type
                                                                            ...
                                                                            <div className="flex justify-end">
                                                                                ...
                                                                                >
                                                                                    ...
                                                                                    <p>
                                                                                        {item.status ===
                                                                                            'ongoing' && (
                                                                                            <span className="w-2 h-2 ml-1 inline-block bg-green-500 animate-pulse rounded-full" />
                                                                                        )}
                                                                                    </p>
                                                                                </Button>
                                                                            </div>
                                            ...
                                            {upcomingEventsData.events.length >
                                                3 && (
                                        ...
                                        <div className="flex flex-col items-center justify-center py-12  mr-3">
                                            ...
                                            <div className="text-left w-6/7 space-y-3 ">
                                                ...
                                                <p className="text-sm text-muted-foreground leading-relaxed">
                                                    Stay tuned! Your upcoming
                                                    assignments, live sessions,
                                                    and deadlines will appear
                                    ...
                                    {(completedClassesData?.classes || [])
                                            ...
                                            <div className="text-center mb-6">
                                                ...
                                                <p className="text-sm text-muted-foreground">
                                                    {completedClassesData
                                                        ...
                                                        0}{' '}
                                                    of{' '}
                                                    {completedClassesData?.totalClasses ||
                                                        0}{' '}
                                                    classes attended
                                                </p>
                                            </div>
                                        ...
                                        <div className="flex flex-col items-center justify-center py-12  mr-3">
                                            ...
                                            <div className="text-left w-6/7 space-y-3 ">
                                                ...
                                                <p className="text-sm text-muted-foreground leading-relaxed">
                                                    Your attendance record will
                                                    appear here once you start
                                                    attending live classes. Stay
                                                    engaged to track your
                                                    progress!
                                                </p>
